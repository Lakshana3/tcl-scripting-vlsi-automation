...
# ----- converts .csv to matrix and creates initial variables "DesignName OutputDirectory NetlistDirectory EarlyLibraryPath LateLibraryPath" ----- #
    ...
    set filename [lindex $argv 0]   ;#Get first command-line argument as CSV filename
    package require csv ;# Load CSV parsing package
    package require struct::matrix  ;# Load matrix structure package
    struct::matrix m    ;# Create a new matrix named “m”
    set f [open $filename]  ;# Open the CSV file for reading
    csv::read2matrix $f m , auto    ;# Read CSV into matrix 'm', using ',' as delimiter, auto-detect types
    close $f    ;# Close the file after reading
    set columns [m columns] ;# Get number of columns in matrix 'm'
    #m add columns $columns ;# Duplicate last column to allow 'link' shape to match dims
    m link my_arr   ;# Link matrix 'm' to array 'my_arr' for easy access
    set num_of_rows [m rows]    ;# Get number of rows in matrix 'm'
    set i 0 ;# Initialize loop counter

    while {$i < $num_of_rows} { ;# Loop over each row
...
# Validate that required files and directories mentioned in csv file exist or not

if {! [file exists $EarlyLibraryPath] } {
 	puts "\nError: Cannot find early cell library in path $EarlyLibraryPath. Exiting ... "
 	exit    ;# Exit on missing file
} else {
...
if {! [file isdirectory $OutputDirectory] } {
 	...
 	file mkdir $OutputDirectory ;# Create directory if missing
} else {
...
if {! [file isdirectory $NetlistDirectory] } {
 	...
 	exit    ;# Exit on missing directory  
} else {
...
::struct::matrix constraints    ;# Create matrix named "constraints"
set chan [open $ConstraintsFile]    ;# Open constraints CSV
csv::read2matrix $chan constraints , auto   ;# Read CSV into matrix 'constraints', using ',' as delimiter, auto-detect types
close $chan ;# Close the file after reading
set number_of_rows [constraints rows]   ;# Store row count
puts "number_of_rows = $number_of_rows"
set number_of_columns [constraints columns] ;# Store column count
puts "number_of_columns = $number_of_columns"

# Locate row & col indices for sections via labeled headers
...
set sdc_file [open $OutputDirectory/$DesignName.sdc "w"]    ;# Create/write SDC file
set i [expr {$clock_start+1}]   ;# Move just past header
set end_of_ports [expr {$input_ports_start-1}]
puts "\nInfo-SDC: Working on clock constraints ....."
while { $i < $end_of_ports } {
    # Construct create_clock & timing commands for each clock port
...
# set up input constraint index positions...
...
while { $i < $end_of_ports } { ;#6
    # Scan v files and differentiating i/p ports as bussed and bits
    set netlist [glob -dir $NetlistDirectory *.v]
    set tmp_file [open /tmp/1 w]
    foreach f $netlist {
    set fd [open $f r]
    while {[gets $fd line] != -1} {
    # Match pattern in Verilog
    ...
    close $tmp_file
    # Read, sort, count unique matched ports
    set tmp_file [open /tmp/1 r]
    # Debug lines using read+split+lsort
    ...
    set count [llength [read $tmp2_file]]   ;# Count matches set count [llength [split [read $tmp2_file] "\n"]]
    ...
    # Generate SDC set_input_delay & transition lines...
...
puts $fileId "\nhierarchy -top $DesignName"
puts $fileId "\nsynth -top $DesignName"
puts $fileId "splitnets -ports -format ___"
puts $fileId "dfflibmap -liberty ${LateLibraryPath}"
puts $fileId "opt"
puts $fileId "abc -liberty ${LateLibraryPath}"
puts $fileId "flatten"
puts $fileId "clean -purge"
puts $fileId "iopadmap -outpad BUFX2 A:Y -bits"
puts $fileId "opt"
puts $fileId "clean"
puts $fileId "write_verilog $OutputDirectory/$DesignName.synth.v"
